...
import { rgbaToHSLA, normalizeRGBA, sortHSL, RGBA, readImageDownsampling, toPixel, readImage } from "../src/util";
...
function run() {
    ...
    cleanResult()
    const DEV_DOWNSAMPLING = false
    const data = toPixel(DEV_DOWNSAMPLING ? readImageDownsampling(img, 100 * 1000)! : readImage(img))
